...
int main() {
  ...
  char game[3][3], **grid = malloc(sizeof(char *) * GRID_LENTH), turn = 'O',
                   _turn = 'X';
  int xyz[5][2][3], input = 1, plays = 0, _pos[5], __pos[5] = {1, 3, 2, 4, 0},
                    pos = 0;
  wins win;
  win.X = 0;
  win.O = 0;
  ...
  while (input) {
    ...
    if (input == KEY_UP || input == KEY_DOWN || input == KEY_LEFT ||
        input == KEY_RIGHT || input == 'D' || input == 'd' || input == 'S' ||
        input == 's' || input == 'A' || input == 'a' || input == 'W' ||
        input == 'w' || input == 'J' || input == 'j' || input == 'K' ||
        input == 'k' || input == 'L' || input == 'l' || input == 'h' ||
        input == 'H') {
    ...
    if (plays >= 5) {
      if (game[0][0] == _turn) {
        if (game[1][1] == _turn) {
          if (game[2][2] == _turn) {
            print_message(&input, &plays, game, _turn, &pos, turn, xyz, grid,
                          __grid, _pos, 0, &win);
          }
        }
    ...
    if (plays == 9) {
      print_message(&input, &plays, game, _turn, &pos, turn, xyz, grid, __grid,
                    _pos, 1, &win);
    }
    print_turn(turn, 1);
  }
